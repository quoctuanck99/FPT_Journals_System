 ...
 */
app.controller('sansimCtrl',['$scope','$http','$filter' ,function ($scope,$http,$filter) {
    $scope.loadContinue=true;
    var pageNumber=1;
    $scope.list={items:"",rowCount:0};
    $scope.page=page;
    $http.get(preUrl+"/admin/history/my-log", {params: {userId:id,p:pageNumber}})
    ...
    $scope.loadPage=function (pageNumber) {
        ...
        $http.get(preUrl+"/admin/history/my-log", {params: {p:pageNumber,userId:id}})
            .then(function (response) {
                    if(response!=null && response!='undefined' && response.status==200){
                        $scope.page=response.data;
